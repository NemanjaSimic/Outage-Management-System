...
namespace NMS.GdaService
{
    ...
    {
        private const string serviceTypeName = "NMS.GdaServiceType";
        ...
        private static void Main()
        {
            ...
            try
            {
                ...
                ServiceRuntime.RegisterServiceAsync(serviceTypeName, context => new GdaService(context)).GetAwaiter().GetResult();

                Logger.LogInformation($"{baseLogString} Main => '{serviceTypeName}' type registered.");
                ServiceEventSource.Current.ServiceTypeRegistered(Process.GetCurrentProcess().Id, typeof(GdaService).Name);
