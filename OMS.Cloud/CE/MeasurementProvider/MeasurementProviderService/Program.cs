...
namespace CE.MeasurementProviderService
{
	...
	{
		private const string serviceTypeName = "CE.MeasurementProviderServiceType";
		...
		private static void Main()
		{
			...
			try
			{
				...
				ServiceRuntime.RegisterServiceAsync(serviceTypeName, context => new MeasurementProviderService(context)).GetAwaiter().GetResult();

				Logger.LogInformation($"{baseLogString} Main => '{serviceTypeName}' type registered.");
				ServiceEventSource.Current.ServiceTypeRegistered(Process.GetCurrentProcess().Id, typeof(MeasurementProviderService).Name);
