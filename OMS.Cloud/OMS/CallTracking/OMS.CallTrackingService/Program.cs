...
namespace OMS.CallTrackingService
{
	...
	{
		private const string serviceTypeName = "OMS.CallTrackingServiceType";
		...
		private static void Main()
		{
			...
			try
			{
				...
				ServiceRuntime.RegisterServiceAsync(serviceTypeName, context => new CallTrackingService(context)).GetAwaiter().GetResult();

				Logger.LogInformation($"{baseLogString} Main => '{serviceTypeName}' type registered.");
				ServiceEventSource.Current.ServiceTypeRegistered(Process.GetCurrentProcess().Id, typeof(CallTrackingService).Name);
