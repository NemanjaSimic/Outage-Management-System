...
namespace OMS.OutageLifecycleService
{
	...
	{
		private const string serviceTypeName = "OMS.OutageLifecycleServiceType";
		...
		private static void Main()
		{
			...
			try
			{
				...
				ServiceRuntime.RegisterServiceAsync(serviceTypeName, context => new OutageLifecycleService(context)).GetAwaiter().GetResult();

				Logger.LogInformation($"{baseLogString} Main => '{serviceTypeName}' type registered.");
				ServiceEventSource.Current.ServiceTypeRegistered(Process.GetCurrentProcess().Id, typeof(OutageLifecycleService).Name);
