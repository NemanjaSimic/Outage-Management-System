...
namespace OMS.HistoryDBManagerService
{
    ...
    {
        private const string serviceTypeName = "OMS.HistoryDBManagerServiceType";
        ...
        private static void Main()
        {
            ...
            try
            {
                ...
                ServiceRuntime.RegisterServiceAsync(serviceTypeName, context => new HistoryDBManagerService(context)).GetAwaiter().GetResult();

                Logger.LogInformation($"{baseLogString} Main => '{serviceTypeName}' type registered.");
                ServiceEventSource.Current.ServiceTypeRegistered(Process.GetCurrentProcess().Id, typeof(HistoryDBManagerService).Name);
